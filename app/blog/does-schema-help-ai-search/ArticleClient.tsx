...
import { Bot, Globe, ArrowRight, Zap, Clock, Sun, Moon, Cpu } from "lucide-react";
    ...
    <div className={`min-h-screen font-sans transition-colors duration-300 antialiased ${!lightTheme ? "bg-cosmic-space bg-cosmic-grid text-white" : "bg-slate-50 text-slate-900"}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-mono tracking-wider font-bold text-accent">SCHEMA & AI GRAPH</span>
            </div>
          </Link>
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-20">
        <div className="flex items-center gap-3 text-xs font-mono mb-4 text-accent">
          <span className="bg-accent/10 border border-accent/30 px-3 py-1 rounded-full font-bold uppercase">Schema.org & Entity Graphs</span>
        ...
        <h1 className="text-3xl sm:text-5xl font-heading font-extrabold tracking-tight leading-[1.2] mb-6 text-ink">
          Does Schema.org Help AI Search Engines Understand Your Website?
        </h1>

        <p className="text-lg sm:text-xl text-ink-2 leading-relaxed mb-8">
          Large Language Models are prone to hallucinations when reading ambiguous prose. JSON-LD Schema.org structured data feeds un-ambiguous facts directly into AI Knowledge Graphs.
        </p>
        ...
        <div className="card p-6 border-l-4 border-l-accent border-border bg-accent/5 rounded-2xl mb-10">
          <div className="text-xs font-mono uppercase text-accent font-bold mb-2 flex items-center gap-2">
            <Cpu className="w-4 h-4 text-accent" /><span>QUICK ANSWER / FACT SHEET</span>
          </div>
          <p className="text-base text-ink leading-relaxed">
            <strong>Yes!</strong> Schema.org JSON-LD is critical for AI search engines. It maps your website elements directly to standardized entities (such as <code className="text-accent font-mono">Organization</code>, <code className="text-accent font-mono">SoftwareApplication</code>, <code className="text-accent font-mono">Product</code>, and <code className="text-accent font-mono">FAQPage</code>), allowing ChatGPT and Perplexity to quote your facts with 100% confidence.
          </p>
        </div>

        <div className="card p-6 border border-border bg-surface rounded-2xl mb-12 shadow-xl">
          <h3 className="text-lg font-heading font-bold mb-2 flex items-center gap-2"><Globe className="w-5 h-5 text-accent" /><span>Validate Your Website's Schema.org Data</span></h3>
          <p className="text-sm text-ink-2 mb-4">Run an instant audit to verify if your site serves clean JSON-LD structured data for AI search models.</p>
          <form onSubmit={handleStartScan} className="flex flex-col sm:flex-row items-center gap-2">
            ...
            <button type="submit" className="btn-primary w-full sm:w-auto px-6 py-3 text-sm font-bold flex items-center justify-center gap-2 shrink-0"><span>Test Schema Data</span><ArrowRight className="w-4 h-4" /></button>
          </form>
        </div>
      </article>

      <footer className="py-8 border-t border-border text-center text-xs text-ink-3 font-mono">
        <p>© {new Date().getFullYear()} AI Scan My Site — Schema & AI Entity Knowledge Guides.</p>
      </footer>
    </div>
  );
}
